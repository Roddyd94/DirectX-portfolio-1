...
private:
    ...
    std::map<std::string, ComPtr<ID2D1Bitmap1>>             _bitmaps;
    ComPtr<ID2D1Factory1>                                   _factory2D = nullptr;

    ComPtr<ID3D11RenderTargetView> _rtvDefault    = nullptr;
    ComPtr<ID3D11DepthStencilView> _dsvDefault    = nullptr;
    ComPtr<ID2D1Bitmap1>           _bitmapDefault = nullptr;
...
public:
    ...
    ComPtr<ID3D11RenderTargetView> GetDefaultRenderTargetView() const { return _rtvDefault; }
    ComPtr<ID3D11DepthStencilView> GetDefaultDepthStencilView() const { return _dsvDefault; }
    ComPtr<ID2D1Bitmap1>           GetDefaultBitmap() const { return _bitmapDefault; }
    ...
    ComPtr<ID3D11RenderTargetView>   FindRenderTargetView(const std::string& name) const;
    ComPtr<ID3D11ShaderResourceView> FindShaderResourceView(const std::string& name) const;
    ComPtr<ID3D11DepthStencilView>   FindDepthStencilView(const std::string& name) const;
    ComPtr<ID2D1Bitmap1>             FindBitmap(const std::string& name) const;

public:
    ComPtr<ID3D11Texture2D> CreateTexture2D(const std::string& name, D3D11_TEXTURE2D_DESC* desc);
    ComPtr<ID3D11RenderTargetView>   CreateRenderTargetView(const std::string& name,
        ComPtr<ID3D11Texture2D>                                                texture = nullptr,
        D3D11_RENDER_TARGET_VIEW_DESC*                                         desc    = nullptr);
    ComPtr<ID3D11RenderTargetView>   CreateRenderTargetView(const std::string& name,
        const std::string&                                                     textureName,
        D3D11_RENDER_TARGET_VIEW_DESC*                                         desc = nullptr);
    ComPtr<ID3D11ShaderResourceView> CreateShaderResourceView(const std::string& name,
      ComPtr<ID3D11Texture2D>                                                    texture,
      D3D11_SHADER_RESOURCE_VIEW_DESC*                                           desc = nullptr);
    ComPtr<ID3D11ShaderResourceView> CreateShaderResourceView(const std::string& name,
      const std::string&                                                         textureName,
      D3D11_SHADER_RESOURCE_VIEW_DESC*                                           desc = nullptr);
    ComPtr<ID3D11DepthStencilView>   CreateDepthStencilView(const std::string& name,
        ComPtr<ID3D11Texture2D>                                                texture,
        D3D11_DEPTH_STENCIL_VIEW_DESC*                                         desc = nullptr);
    ComPtr<ID3D11DepthStencilView>   CreateDepthStencilView(const std::string& name,
        const std::string&                                                     textureName,
        D3D11_DEPTH_STENCIL_VIEW_DESC*                                         desc = nullptr);
    ComPtr<ID2D1Bitmap1>        CreateBitmap(
